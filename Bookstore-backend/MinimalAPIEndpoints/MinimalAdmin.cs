...
public static class MinimalAdminEndpoint
{
    ...
    ///    
    /// </remarks>
    private static async Task<IResult> InsertBook([FromBody] BookinsertModel bodydata, ICrudlayer dbcall)
    {

        if (bodydata.PublicationDate.year > DateTime.UtcNow.Year) return Results.BadRequest("wrong year");
        ...
        return Results.Json(message, statusCode: message.Code);
    }
    ...
    public static async Task<IResult> DeletebyISBN(
        [FromRoute][RegularExpression("^[0-9]*$")] string ISBN,
        ICrudlayer dbcall,
        CancellationToken ctoken)
    {
